...
optional.add_argument("--prodsteps",  type=int, help="production steps", default=int(1e6))                      #steps for the production phase during the md
...
#training procedure.
calc = NNCalculator(
    checkpoint="./models/model-XXXXX/YYYYY", #load the model you want to use
    atoms=atoms,
    charge=args.charge,
    F=128,
    K=64,
...
#calc = XTB(method='GFN2-xTB', charge=args.charge)
...
for i in range(args.equilsteps):
    ...
    if i % 100 == 0:
        ...
        etot = epot + ekin
        print("Total energy:", etot.item(), " eV")
